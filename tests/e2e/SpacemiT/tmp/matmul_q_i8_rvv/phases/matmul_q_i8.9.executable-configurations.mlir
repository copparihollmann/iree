#config = #iree_cpu.lowering_config<vector_common_parallel = [1, 1, 8, 16]>
#config1 = #iree_cpu.lowering_config<distribution = [1, 1, 0, 0, 0, 0], vector_common_parallel = [1, 1, 0, 8, 16, 0], vector_reduction = [0, 0, 1, 0, 0, 1]>
#config2 = #iree_cpu.lowering_config<vector_common_parallel = [1, 1]>
#config3 = #iree_cpu.lowering_config<distribution = [1, 16], vector_common_parallel = [1, 1]>
    ...
    hal.executable.variant public @embedded_elf_riscv_64 target(#executable_target_embedded_elf_riscv_64) {
      ...
      builtin.module {
        func.func @matmul_i8_quantized_dispatch_0_matmul_32x32x64_i8xi8xi32() attributes {translation_info = #translation} {
          ...
          %0 = hal.interface.binding.subspan layout(#pipeline_layout) binding(0) alignment(64) offset(%c0) flags("ReadOnly|Indirect") : !iree_tensor_ext.dispatch.tensor<readonly:tensor<4x64x8x1xi8>>
          %1 = hal.interface.binding.subspan layout(#pipeline_layout) binding(0) alignment(64) offset(%c2048) flags("ReadOnly|Indirect") : !iree_tensor_ext.dispatch.tensor<readonly:tensor<2x64x16x1xi8>>
          ...
          %3 = iree_tensor_ext.dispatch.tensor.load %0, offsets = [0, 0, 0, 0], sizes = [4, 64, 8, 1], strides = [1, 1, 1, 1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<4x64x8x1xi8>> -> tensor<4x64x8x1xi8>
          ...
          %5 = tensor.empty() : tensor<4x2x8x16xi32>
          %6 = linalg.fill {lowering_config = #config} ins(%c0_i32 : i32) outs(%5 : tensor<4x2x8x16xi32>) -> tensor<4x2x8x16xi32>
          %7 = linalg.mmt4d {lowering_config = #config1} ins(%3, %4 : tensor<4x64x8x1xi8>, tensor<2x64x16x1xi8>) outs(%6 : tensor<4x2x8x16xi32>) -> tensor<4x2x8x16xi32>
          %8 = tensor.empty() : tensor<32x32xi32>
          %unpack = linalg.unpack %7 outer_dims_perm = [0, 1] inner_dims_pos = [0, 1] inner_tiles = [8, 16] into %8 {lowering_config = #config2} : tensor<4x2x8x16xi32> -> tensor<32x32xi32>
    ...
    hal.executable.variant public @embedded_elf_riscv_64 target(#executable_target_embedded_elf_riscv_64) {
      ...
      builtin.module {
        func.func @_encoding_0_encode_32x64xi8_to_32x64xi8() attributes {translation_info = #translation1} {
          ...
          %1 = hal.interface.binding.subspan layout(#pipeline_layout) binding(1) alignment(64) offset(%c0) flags(Indirect) : !iree_tensor_ext.dispatch.tensor<writeonly:tensor<4x64x8x1xi8>>
          ...
          %3 = tensor.empty() : tensor<4x64x8x1xi8>
          %pack = linalg.pack %2 outer_dims_perm = [0, 1] inner_dims_pos = [0, 1] inner_tiles = [8, 1] into %3 {lowering_config = #config3} : tensor<32x64xi8> -> tensor<4x64x8x1xi8>
          iree_tensor_ext.dispatch.tensor.store %pack, %1, offsets = [0, 0, 0, 0], sizes = [4, 64, 8, 1], strides = [1, 1, 1, 1] : tensor<4x64x8x1xi8> -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<4x64x8x1xi8>>
          return
        }
      }
    }
    ...
    hal.executable.variant public @embedded_elf_riscv_64 target(#executable_target_embedded_elf_riscv_64) {
      ...
      builtin.module {
        func.func @_encoding_1_encode_64x32xi8_to_64x32xi8() attributes {translation_info = #translation1} {
          ...
          %1 = hal.interface.binding.subspan layout(#pipeline_layout) binding(1) alignment(64) offset(%c2048) flags(Indirect) : !iree_tensor_ext.dispatch.tensor<writeonly:tensor<2x64x16x1xi8>>
          ...
          %pack = linalg.pack %2 outer_dims_perm = [1, 0] inner_dims_pos = [1, 0] inner_tiles = [16, 1] into %3 {lowering_config = #config4} : tensor<64x32xi8> -> tensor<2x64x16x1xi8>

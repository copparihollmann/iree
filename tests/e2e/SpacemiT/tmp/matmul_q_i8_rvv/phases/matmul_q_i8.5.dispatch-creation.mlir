...
module attributes {stream.affinity.default = #hal.device.affinity<@__device_0>} {
  ...
  util.func public @matmul_i8_quantized(%arg0: !hal.buffer_view, %arg1: !hal.buffer_view) -> !hal.buffer_view attributes {iree.abi.stub, iree.reflection = {iree.abi.declaration = "sync func @matmul_i8_quantized(%input0: tensor<32x64xi8>, %input1: tensor<64x32xi8>) -> (%output0: tensor<32x32xi32>)"}} {
    ...
    %4 = flow.dispatch.workgroups(%2, %3) : (tensor<32x64xi8, #encoding>, tensor<64x32xi8, #encoding1>) -> tensor<32x32xi32> =
        (%arg2: !iree_tensor_ext.dispatch.tensor<readonly:tensor<32x64xi8, #encoding>>, %arg3: !iree_tensor_ext.dispatch.tensor<readonly:tensor<64x32xi8, #encoding1>>, %arg4: !iree_tensor_ext.dispatch.tensor<writeonly:tensor<32x32xi32>>) {
      %c0_i32 = arith.constant 0 : i32
      %6 = iree_tensor_ext.dispatch.tensor.load %arg2, offsets = [0, 0], sizes = [32, 64], strides = [1, 1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<32x64xi8, #encoding>> -> tensor<32x64xi8, #encoding>
      %7 = iree_tensor_ext.dispatch.tensor.load %arg3, offsets = [0, 0], sizes = [64, 32], strides = [1, 1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<64x32xi8, #encoding1>> -> tensor<64x32xi8, #encoding1>
      %8 = tensor.empty() : tensor<32x32xi32, #encoding2>
      %9 = linalg.fill ins(%c0_i32 : i32) outs(%8 : tensor<32x32xi32, #encoding2>) -> tensor<32x32xi32, #encoding2>
      %10 = linalg.matmul ins(%6, %7 : tensor<32x64xi8, #encoding>, tensor<64x32xi8, #encoding1>) outs(%9 : tensor<32x32xi32, #encoding2>) -> tensor<32x32xi32, #encoding2>
      %11 = iree_encoding.unset_encoding %10 : tensor<32x32xi32, #encoding2> -> tensor<32x32xi32>
      iree_tensor_ext.dispatch.tensor.store %11, %arg4, offsets = [0, 0], sizes = [32, 32], strides = [1, 1] : tensor<32x32xi32> -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<32x32xi32>>
      flow.return

...
module attributes {stream.affinity.default = #hal.device.affinity<@__device_0>} {
  ...
  stream.executable private @matmul_fp8_dispatch_0 {
    stream.executable.export public @matmul_fp8_dispatch_0_matmul_32x32x64_f8E4M3FNxf8E4M3FNxf16 workgroups() -> (index, index, index) {
      %x, %y, %z = iree_tensor_ext.dispatch.workgroup_count_from_slice()
      stream.return %x, %y, %z : index, index, index
    }
    builtin.module {
      func.func @matmul_fp8_dispatch_0_matmul_32x32x64_f8E4M3FNxf8E4M3FNxf16(%arg0: !stream.binding {stream.alignment = 64 : index}, %arg1: !stream.binding {stream.alignment = 64 : index}) {
        %cst = arith.constant 0.000000e+00 : f16
        %c0 = arith.constant 0 : index
        %c2048 = arith.constant 2048 : index
        %0 = stream.binding.subspan %arg0[%c0] : !stream.binding -> !iree_tensor_ext.dispatch.tensor<readonly:tensor<32x64xf8E4M3FN, #encoding>>
        %1 = stream.binding.subspan %arg0[%c2048] : !stream.binding -> !iree_tensor_ext.dispatch.tensor<readonly:tensor<64x32xf8E4M3FN, #encoding>>
        %2 = stream.binding.subspan %arg1[%c0] : !stream.binding -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<32x32xf16>>
  ...
  stream.executable private @_encoding_0 {
    stream.executable.export public @_encoding_0_encode_32x64xf8E4M3FN_to_32x64xf8E4M3FN workgroups() -> (index, index, index) {
      %x, %y, %z = iree_tensor_ext.dispatch.workgroup_count_from_slice()
      stream.return %x, %y, %z : index, index, index
    }
    builtin.module {
      func.func @_encoding_0_encode_32x64xf8E4M3FN_to_32x64xf8E4M3FN(%arg0: !stream.binding {stream.alignment = 64 : index}, %arg1: !stream.binding {stream.alignment = 64 : index}) {
        %c0 = arith.constant 0 : index
        %0 = stream.binding.subspan %arg0[%c0] : !stream.binding -> !iree_tensor_ext.dispatch.tensor<readonly:tensor<32x64xf8E4M3FN>>
        %1 = stream.binding.subspan %arg1[%c0] : !stream.binding -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<32x64xf8E4M3FN, #encoding>>
  ...
  stream.executable private @_encoding_1 {
    stream.executable.export public @_encoding_1_encode_64x32xf8E4M3FN_to_64x32xf8E4M3FN workgroups() -> (index, index, index) {
      %x, %y, %z = iree_tensor_ext.dispatch.workgroup_count_from_slice()
      stream.return %x, %y, %z : index, index, index
    }
    builtin.module {
      func.func @_encoding_1_encode_64x32xf8E4M3FN_to_64x32xf8E4M3FN(%arg0: !stream.binding {stream.alignment = 64 : index}, %arg1: !stream.binding {stream.alignment = 64 : index}) {
        %c0 = arith.constant 0 : index
        %c2048 = arith.constant 2048 : index
        %0 = stream.binding.subspan %arg0[%c0] : !stream.binding -> !iree_tensor_ext.dispatch.tensor<readonly:tensor<64x32xf8E4M3FN>>
        %1 = stream.binding.subspan %arg1[%c2048] : !stream.binding -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<64x32xf8E4M3FN, #encoding>>
  ...
  util.func public @matmul_fp8(%arg0: !hal.buffer_view, %arg1: !hal.buffer_view) -> !hal.buffer_view attributes {iree.abi.stub, iree.reflection = {iree.abi.declaration = "sync func @matmul_fp8(%input0: tensor<32x64xf8E4M3FN>, %input1: tensor<64x32xf8E4M3FN>) -> (%output0: tensor<32x32xf16>)"}} {
    ...
    %3 = stream.cmd.execute on(#hal.device.affinity<@__device_0>) await(%2) => with(%0 as %arg2: !stream.resource<external>{%c2048}, %1 as %arg3: !stream.resource<external>{%c2048}, %result as %arg4: !stream.resource<external>{%c2048}, %result_0 as %arg5: !stream.resource<transient>{%c4096}) {
      stream.cmd.concurrent {
        stream.cmd.dispatch @_encoding_0::@_encoding_0_encode_32x64xf8E4M3FN_to_32x64xf8E4M3FN {
          ro %arg2[%c0 for %c2048] : !stream.resource<external>{%c2048},
          wo %arg5[%c0 for %c4096] : !stream.resource<transient>{%c4096}
        }
        stream.cmd.dispatch @_encoding_1::@_encoding_1_encode_64x32xf8E4M3FN_to_64x32xf8E4M3FN {
          ro %arg3[%c0 for %c2048] : !stream.resource<external>{%c2048},
          wo %arg5[%c0 for %c4096] : !stream.resource<transient>{%c4096}
        }
      }
      stream.cmd.dispatch @matmul_fp8_dispatch_0::@matmul_fp8_dispatch_0_matmul_32x32x64_f8E4M3FNxf8E4M3FNxf16 {
